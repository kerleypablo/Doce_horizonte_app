...
export const useOrderData = (token: string | undefined, orderId: string | undefined, detailEnabled: boolean) => {
  const enabled = Boolean(token);
  const ordersQuery = useCachedQuery(queryKeys.orders, () => orderService.list(token), {
    staleTime: 60_000, enabled, refetchInterval: 90_000
  });
