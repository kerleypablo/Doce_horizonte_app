...
alter table public.company_settings
  ...
  add column if not exists app_theme text not null default 'caramelo',
  add column if not exists dark_mode boolean not null default false,
  add column if not exists default_notes_delivery text not null default '',
  add column if not exists default_notes_general text not null default '',
  add column if not exists default_notes_payment text not null default '',
  add column if not exists productive_hours_per_month numeric not null default 0,
  add column if not exists overhead_method text not null default 'PERCENT_DIRECT',
  add column if not exists overhead_percent numeric not null default 0,
  add column if not exists overhead_per_unit numeric not null default 0,
  ...
  add column if not exists taxes_percent numeric not null default 0,
  add column if not exists default_profit_percent numeric not null default 0;
